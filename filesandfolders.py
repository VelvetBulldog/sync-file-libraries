import os, win32con, win32file, filecopy, attribs, queryMSdb
...
def folderwalk(toplevelsource, topleveldest):
    ...
    destdict = queryMSdb.MSSQLdict()

    for root, dirs, files in os.walk(toplevelsource):
        path = root.split(os.sep)
        if path[-1] == 'Archive':
            print("The directory ending in Archive folder: ",root)
            ...
            print("This is the destination directory: ", destination)
            for file in files:
                dirpath = os.path.join(root, file)
                print("This is the file and path that is about to be moved: ",dirpath) #print the file that is about to be worked on
                ...
                if attribs.fileattributecheck(source, win32con.FILE_ATTRIBUTE_ARCHIVE):
                    filecopy.cpfile(source, destination, lz_directory, destdict, file) #copy file from source to destination
